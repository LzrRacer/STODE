...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Analyze and plot snapshots of spatiotemporal simulations.")
    parser.add_argument("--simulation_dir", type=str, required=True, help="Path to the simulation results directory (containing .npz trajectory).")
    ...
    parser.add_argument("--clustered_adata_path", type=str, required=True, help="Path to the AnnData object with cluster assignments (e.g., from script 05).")
    parser.add_argument("--output_dir", type=str, required=True, help="Directory to save analysis plots.")
    parser.add_argument("--timepoints_to_analyze", type=float, nargs='+', required=True, help="List of biological timepoints to plot (e.g., 8.5 9.5 10.5).")
    parser.add_argument("--latent_compress_method", type=str, default='umap', choices=['umap', 'pca'], help="Method for compressing latent space for visualization.")
    parser.add_argument('--latent_compress_subsample_fit', type=int, default=10000, help="Number of cells to subsample for fitting UMAP. Set to 0 to disable. Default: 10000.")
    parser.add_argument("--spot_size", type=float, default=1, help="Size of the spots in spatial plots.")
    parser.add_argument("--cluster_key", type=str, default="kmeans_cluster", help="The key in adata.obs that stores the cluster labels.")
    
    # Correctly defined arguments (no duplicates)
...
def main(args):
    ...
    # --- Pre-compute Global Latent Embedding ---
    print("Pre-computing global 1D latent embedding...")
    all_latent_states = np.vstack([arr[:, spatial_dim:] for arr in traj_list if arr.shape[0] > 0])
    global_latent_1d = compress_to_1d(
        all_latent_states,
    ...
    # --- OPTIMIZED: Estimate Potential Range via Subsampling ---
    ...
    potential_vmin = np.percentile(sampled_potentials, 5)
    potential_vmax = np.percentile(sampled_potentials, 95)
    latent_vmin, latent_vmax = 0, 1
    ...
    cluster_key = args.cluster_key # Use the argument instead of the hardcoded string
    cluster_names = adata_clustered.obs[cluster_key].cat.categories.tolist()
    cluster_colors = adata_clustered.uns.get(f'{cluster_key}_colors', None)
    
    if isinstance(cluster_colors, np.ndarray):
    ...
    for target_time in args.timepoints_to_analyze:
        try:
            ...
            if state_t.shape[0] == 0:
                print("  -> No cells to plot for this timepoint.")
                continue
            ...
            num_cells_t = latent_t.shape[0]
            adata_snapshot.obs[f'latent_1d_{args.latent_compress_method}'] = global_latent_1d[latent_offset : latent_offset + num_cells_t]
            ...
            latent_offset += num_cells_t

            # --- Generate Plots ---
            time_str_safe = f"{actual_sim_time:.2f}".replace('.', 'p')
            
            sc.pl.spatial(adata_snapshot, color="potential_U", cmap="viridis",
                          title=f"Potential (U) at t={actual_sim_time:.2f}",
                          vmin=potential_vmin, vmax=potential_vmax,
                          save=f"_snapshot_{time_str_safe}_potential.png", show=False, spot_size=args.spot_size)
            plt.close()

            sc.pl.spatial(adata_snapshot, color=f"latent_1d_{args.latent_compress_method}", cmap="magma",
                          title=f"Latent 1D UMAP at t={actual_sim_time:.2f}",
                          vmin=latent_vmin, vmax=latent_vmax,
                          save=f"_snapshot_{time_str_safe}_latent1D.png", show=False, spot_size=args.spot_size)
            plt.close()

            sc.pl.spatial(adata_snapshot, color=cluster_key, palette=cluster_colors,
                          title=f"Clusters at t={actual_sim_time:.2f}",
                          save=f"_snapshot_{time_str_safe}_clusters.png", show=False, spot_size=args.spot_size)
            plt.close()
            
            print(f"  ✓ Plots for t={actual_sim_time:.2f} saved to {output_dir}")
